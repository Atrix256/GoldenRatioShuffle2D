...
void DoTest2D(const uint2& dims, std::mt19937& rng, uint seed)
{
	printf("========== 2D: %u x %u ==========\n\n", dims[0], dims[1]);
	...
	if (true)
	{
		...
		uint valueI = Clamp<uint>((uint)(startValue[0] * float(dims[0] * dims[1])), 0, dims[0] * dims[1] - 1);

		LDShuffle shuffle(dims[0] * dims[1], seed);

		DoTest2D_Single(dims,
			[&valueI, &dims, coprime, &image, &image2, &shuffle](uint index, float percent)
			{
				...
				int x = 0;
				int y = 0;
				d2xy(dims[0] * dims[1], shuffledIndex, &x, &y);
...
// This slack chat: https://electronic-arts.slack.com/archives/GCML9HEEN/p1716221742406909
